...
const updateCreatorCred = async (creator_id, new_email, new_pass) => {
  const res = await fetch(`/api/creator?id=${creator_id}`, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({ email: new_email, password: new_pass }),
  });
  return await res.json();
};

const getAllCourses = async () => {
  const coursesjson = await fetch("/api/course");
  return await coursesjson.json();
};
